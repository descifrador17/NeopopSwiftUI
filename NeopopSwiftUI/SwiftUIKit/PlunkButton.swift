...
struct PlunkButton: ButtonStyle {
    ...
    func makeBody(configuration: Configuration) -> some View {
        ZStack(alignment: calcZStackAlignment(buttonDirection: buttonModel.buttonDirection)){
            if !configuration.isPressed {
                HStack(alignment: calcHStackAlignment(buttonDirection: buttonModel.buttonDirection), spacing: 0){
                    //left edge
...
struct PlunkButton_Previews: PreviewProvider {
    
    static var previews: some View {
        HStack{
            Button("Press Me"){
                print("hello")
            }.buttonStyle(PlunkButton(buttonModel: NeopopHelper.primary50_plunk_button_model(style: .dark), buttonState: .normal))
        }
